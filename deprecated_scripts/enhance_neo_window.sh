...
# ==================================================================================
# Enhancement Script for the NEO Window
# This implements a master-detail view for exploring NEO data.
# ==================================================================================

echo "--- Enhancing the NEO Window with a Detail View ---"
                    ...
                    <ul>
                        {neoList.map(neo => (
                            <li 
                                ...
                                className={`cursor-pointer mb-1 p-1 ${selectedNeo?.id === neo.id ? 'bg-s7-blue text-white' : 'hover:bg-s7-blue hover:text-white'}`}
                            >
                                {neo.name}
                            </li>
                        ))}
                ...
                {selectedNeo && closeApproach ? (
                    <>
                        ...
                        <div className="text-xs overflow-y-auto">
                            ...
                            <DetailRow label="Orbital Period" value={`${parseFloat(selectedNeo.orbital_data.orbital_period).toFixed(0)} days`} />
                            <DetailRow label="First Observation" value={selectedNeo.orbital_data.first_observation_date} />
                        </div>
...
echo ""
echo "✅ NEO Window Enhanced. Restart your client to see the changes."